...
runs=5
rps=(10 20 30 40 50 60 70 80 100)
...
throughput() {
  ...
  kubectl get pods --no-headers -o custom-columns=":metadata.name" | grep "^${function_name}-${runtime}" | while read -r pod_name; do
    ...
    kubectl describe pod "$pod_name" | grep "Container ID" | head -n 1 | awk '{print $3}' | sed 's|containerd://||' | sed 's|^|Container ID: |' >>"$file_info"

    #kubectl describe pod "$pod_name" | grep "Container ID" | head -n 1 | awk '{print $3}' | sed 's|containerd://||' | sed 's|^|Container ID: |'
