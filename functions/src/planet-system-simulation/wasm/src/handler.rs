...
pub async fn handle_json(json: Value) -> String {
    let result: Result<Json, _> = serde_json::from_value(json);

    match result {
        Ok(valid_json) => {
            let (energy_before, energy_after) = run(valid_json.n);

            let formatted_energy_before = format!("{:.9}", energy_before);
            let formatted_energy_after = format!("{:.9}", energy_after);

            let response = json!({
                "status": "success",
                "runtime": "wasm",
                "data": {
                    "energy-before": formatted_energy_before,
                    "energy-after": formatted_energy_after
                }
            });
